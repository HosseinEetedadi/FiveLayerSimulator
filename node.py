...
class Node:
    ...
    def __init__(self, env, uid, session_uid,level,
                 ...
                 net_speed_scheduler_UPscheduler_mbits=1500,
                 net_speed_UPscheduler_cloud_mbits=1000,
                 net_speed_UPscheduler_UPworker_mbits=200000,
        ...
        # processes
        if self._node_type == Node.NodeType.WORKER or self._node_type == Node.NodeType.UP_WORKER or self._node_type == Node.NodeType.High_WORKER:
    ...
    def init(self):
        ...
        if self.get_level() == 3:
            workers_n = len(self._service_discovery.get_workers_in_high_cluster(self._node_belong_to_cluster))
    ...
    def _process_job_executor(self):
        ...
        while True:
            try:
                ...
                # print(f"This is the machine type = {self._node_type} and machine speed = {self._machine_speed}")
...
# /////*****///////
                    ...
                    yield self._env.timeout(tt)
                    # set the next action
                    job.set_transmission_next_action(Job.TransmissionAction.HIGH_WORKER_TO_HIGH_SCHEDULER)

                    self._service_discovery.get_high_node_by_uid(job.get_forwarded_to_node_id()).schedule_job(job)
#  /////*****///////
...
# // // / ** ** * // // // /
                    ...
                    yield self._env.timeout(tt)
                    # set the next action
                    job.set_transmission_next_action(Job.TransmissionAction.CLOUD_TO_HIGH_SCHEDULER)
                    # schedule job to the worker
                    # print(f"CLOUD => {self._service_discovery.get_node_cloud().schedule_job(job, self.get_level())}")
                    self._service_discovery.get_node_cloud().schedule_job(job, self.get_level())
#/////*****///////
...
#/////*****///////
                    ...
                    # retrieve the scheduler node of the cluster
                    cluster_scheduler_node = self._service_discovery.get_node_high_scheduler_for_cluster_id(
                        job.get_forwarded_to_cluster_id())
...
# /////*****///////
                ...
                elif next_action == Job.TransmissionAction.UPWORKER_TO_UPSCHEDULER or next_action == Job.TransmissionAction.HIGH_WORKER_TO_HIGH_SCHEDULER or\
                        next_action == Job.TransmissionAction.WORKER_TO_SCHEDULER or next_action == Job.TransmissionAction.CLOUD_TO_HIGH_SCHEDULER or\
...
# //****//
    def _job_forward_to_high_worker(self, job, to_node: Node):
        job.set_transmission_next_action(Job.TransmissionAction.HIGH_SCHEDULER_TO_HIGH_WORKER)
        job.a_forwarded(to_node.get_uid())
        self._job_transmit(job)
...
# ///***///
    def _job_forward_to_up_cluster(self, job):
        ...
        to_cluster_uid = self._service_discovery.get_clusters_count() - 2
        ...
        self._job_transmit(job)
    # ///***///
    def _job_forward_to_high_cluster(self, job, level):
        # in order to find up cluster index
        to_cluster_uid = self._service_discovery.get_clusters_count() - 1
        if level == 1:
            job.set_transmission_next_action(Job.TransmissionAction.SCHEDULER_TO_HIGH_CLUSTER)
        elif level == 2:
            job.set_transmission_next_action(Job.TransmissionAction.UPSCHEDULER_TO_HIGH_CLUSTER)

        # retrieve cluster id from index
        # retrieve the scheduler for that cluster
        scheduler_node = self._service_discovery.get_node_high_scheduler_for_cluster_id(to_cluster_uid)
        # forward
        job.a_forwarded(scheduler_node.get_uid(), to_cluster_id = to_cluster_uid, to_up=2)
        self._job_transmit(job)
    ...
    def _job_schedule(self, job):
        ...
        if self.get_current_load() >= self._max_jobs_in_queue:
            ...
            # print(f"node = {self._uid}, level = {self.get_level()}")
            self.reject_count += 1
            self._job_reject(job)
            return False
    ...
    def _get_actions(self):
        ...
        if self.get_level() == 1:
            ...
            nodes_workers = self._service_discovery.get_workers_in_high_cluster(self._node_belong_to_cluster)
        if self._actions_space is Node.ActionsSpace.ONLY_WORKERS:
            ...
            # actions space = [REJECT, CLOUD, Up_layer, High_layer, W1, W2, W3, C1, C2, ...]
            if self.get_level() == 1:
                return [i for i in range(4 + len(nodes_workers) + self._service_discovery.get_clusters_count() - 1)]
            elif self.get_level() == 2:
                return [i for i in range(3 + len(nodes_workers))]
            elif self.get_level() == 3:
                return [i for i in range(2 + len(nodes_workers))]
        else:
    ...
    def _get_possible_actions(self, job: Job, state: List[int]):
        ...
        if self.get_level() == 1:
            ...
            n_workers = self._service_discovery.get_workers_in_high_cluster_count(self._node_belong_to_cluster)
    ...
    def _act_execute(self, action: int, job: Job):
        ...
        if self.get_level() == 1:
            ...
            worker_nodes = self._service_discovery.get_workers_in_high_cluster(self._node_belong_to_cluster)
        ...
        if self._actions_space == Node.ActionsSpace.ONLY_WORKERS:
            ...
            if self.get_level() == 1:
                if action == 0:
                    ...
                    return self._job_forward_to_cloud(job)
                elif action == 2 or (action - 4 - worker_nodes_n) == 3:
                    # print(f"job = {job.get_uid()}   THE ACTION VALUE IS 2")
                    return self._job_forward_to_up_cluster(job)
                elif action == 3 or (action - 4 - worker_nodes_n) == 4:
                    # print(f"job = {job.get_uid()}   THE ACTION VALUE IS 3")
                    return self._job_forward_to_high_cluster(job, 1)
                elif 3 < action < worker_nodes_n + 4:
                    # print(f"The level is 1, action = {action}, length of worker nodes = {len(worker_nodes)}")
                    return self._job_forward_to_worker(job, worker_nodes[action - 4])
                else:
                    # print(f"this is the  action = {action}\nthis is the worker nodes number = {worker_nodes_n}")
                    cluster_index = action - 4 - worker_nodes_n
                ...
                if action == 0:
                    ...
                    return self._job_forward_to_high_worker(job, worker_nodes[action - 2])

        else:
            Log.mfatal(f"{MODULE}#{self._uid}", f"_act_fexecute: action space {self._actions_space} is not valid")
            sys.exit(1)
    ...
    def _init_d_sarsa(self):
        ...
        if self._node_type == Node.NodeType.SCHEDULER or self._node_type == Node.NodeType.UP_SCHEDULER or self._node_type == Node.NodeType.High_SCHEDULER:
    ...
    def _d_sarsa_learn_episode(self):
        ...
        while len(self._scheduled_jobs) > 0 and self._scheduled_jobs[0].get_episode() == episode:
            ...
            # print(f"Vanish ==>> {self._node_type}")
