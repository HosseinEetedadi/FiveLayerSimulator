...
class ServiceDiscovery:
    ...
    def __init__(self, n_clusters, n_level, nodes, node_cloud):
        ...
        for node in nodes:
            ...
            elif node.get_level() == 3:
                self._High_nodes_id_dict[node.get_uid()] = node
                self._High_clusters_allocaltion[node.get_cluster_id()].append(node.get_uid())
                if node.get_type() == Node.NodeType.High_SCHEDULER:
                    self._High_cluster_schedulers[node.get_cluster_id()] = node.get_uid()

        for cluster_i in range(self._n_clusters - 1):
            self._clusters_allocation[cluster_i] = sorted(self._clusters_allocation[cluster_i])

        self._Up_clusters_allocation[self._n_clusters - 2] = sorted(self._Up_clusters_allocation[self._n_clusters - 2])
        self._High_clusters_allocaltion[self._n_clusters - 1] = sorted(self._High_clusters_allocaltion[self._n_clusters - 1])
    ...
    def get_high_node_by_uid(self, node_id):
        """Return a node by its id"""
        if node_id not in self._High_nodes_id_dict.keys():
            raise RuntimeError(f"Requested node_id={node_id} is non-existent")
            # return None
        return self._High_nodes_id_dict[node_id]
    ...
    def get_workers_in_high_cluster(self, cluster_id):
        nodes_in_high_cluster = []
        for node_uid in self._High_clusters_allocaltion[cluster_id]:
            if self.get_high_node_by_uid(node_uid).get_type() == Node.NodeType.High_WORKER:
                nodes_in_high_cluster.append(self.get_high_node_by_uid(node_uid))
        return nodes_in_high_cluster
    ...
    def get_workers_in_high_cluster_count(self, cluster_id):
        return len(self.get_workers_in_high_cluster(cluster_id))
    ...
    def get_node_high_scheduler_for_cluster_id(self, cluster_id):
        """Retrieve the scheduler node uid from the cluster uid"""
        return self.get_high_node_by_uid(self._High_cluster_schedulers[cluster_id])

    def get_cluster_uid_from_index(self, current_cluster_id, cluster_index):
        """Convert a cluster index to cluster uid"""
        # print(f"This is the current_cluster_id = {current_cluster_id}")
        ...
        # print(f"after - this is the clusters_arr: {clus/ters_arr}")
