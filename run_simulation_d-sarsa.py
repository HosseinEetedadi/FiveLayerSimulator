...
def simulate(env):
    ...
    nodes.append(create_node(env, 2, 0, 1, Node.NodeType.WORKER, 0.4))
    nodes.append(create_node(env, 3, 0, 1, Node.NodeType.WORKER, 0.3))
    nodes.append(create_node(env, 4, 0, 1, Node.NodeType.WORKER, 0.2))

    if MULTI_CLUSTER:
        nodes.append(create_node(env, 5, 1, 1, Node.NodeType.SCHEDULER, 1.0))
        nodes.append(create_node(env, 6, 1, 1, Node.NodeType.WORKER, 0.2))
        nodes.append(create_node(env, 7, 1, 1, Node.NodeType.WORKER, 0.3))  # ////
        nodes.append(create_node(env, 8, 1, 1, Node.NodeType.WORKER, 0.4))
        nodes.append(create_node(env, 9, 1, 1, Node.NodeType.WORKER, 0.2))

        nodes.append(create_node(env, 10, 2, 1, Node.NodeType.SCHEDULER, 1.0))
        nodes.append(create_node(env, 11, 2, 1, Node.NodeType.WORKER, 0.2))
        nodes.append(create_node(env, 12, 2, 1, Node.NodeType.WORKER, 0.4))  # ////
        nodes.append(create_node(env, 13, 2, 1, Node.NodeType.WORKER, 0.2))

        nodes.append(create_node(env, 14, 3, 2, Node.NodeType.UP_SCHEDULER, 1))
        nodes.append(create_node(env, 15, 3, 2, Node.NodeType.UP_WORKER, 1))
        nodes.append(create_node(env, 16, 3, 2, Node.NodeType.UP_WORKER, 1))
        nodes.append(create_node(env, 17, 3, 2, Node.NodeType.UP_WORKER, 1))
        nodes.append(create_node(env, 18, 3, 2, Node.NodeType.UP_WORKER, 1))

        nodes.append(create_node(env, 19, 4, 3, Node.NodeType.High_SCHEDULER, 1))
        nodes.append(create_node(env, 20, 4, 3, Node.NodeType.High_WORKER, 1))
        nodes.append(create_node(env, 21, 4, 3, Node.NodeType.High_WORKER, 1))
        nodes.append(create_node(env, 22, 4, 3, Node.NodeType.High_WORKER, 1))
        nodes.append(create_node(env, 23, 4, 3, Node.NodeType.High_WORKER, 1))
    # add them discovery service
    discovery = ServiceDiscovery(5, 3, nodes, cloud)
    ...
    print(f"Number of rejected jobs = {reject_count}")
...
def create_node(env, node_id, belong_to_cluster_id, level, node_type, machine_speed):
    return Node(env,
                ...
                net_speed_UPscheduler_UPworker_mbits=2000,
